<div class="container conceptos hide-until-dom-ready hidden-sm hidden-xs visible-lg visible-md">
    <div class="row">
        <div class="content col-md-12">
            <div class="row islas-container">
                <div class="sentir isla-background">

                    <div class="concepto-texto-container">
                        <div class="concepto-texto">
                            <span><?php echo $isla_sentir_texto ?></span>
                        </div>
                    </div>
                    <div class="concepto-titulo-container <?php echo $idioma_ext?>isla-sentir-titulo">
<!--                        <h4 class="concepto-titulo">-->
<!--                                --><?php //echo $isla_sentir_titulo ?>
<!--                        </h4>-->

                    </div>
                </div>
                <div class="descubrir isla-background">

                    <div class="concepto-texto-container">
                        <div class="concepto-texto">
                            <span><?php echo $isla_descubrir_texto ?></span>
                        </div>
                    </div>
                    <div class="concepto-titulo-container <?php echo $idioma_ext?>isla-descubrir-titulo">
<!--                        <h4 class="concepto-titulo">-->
<!--                                --><?php //echo $isla_descubrir_titulo ?>
<!--                        </h4>-->

                    </div>
                </div>
                <div class="crear isla-background">

                    <div class="concepto-texto-container">
                        <div class="concepto-texto">
                            <span><?php echo $isla_crear_texto ?></span>
                        </div>
                    </div>
                    <div class="concepto-titulo-container <?php echo $idioma_ext?>isla-crear-titulo">
<!--                        <h4 class="concepto-titulo">-->
<!--                                --><?php //echo $isla_crear_titulo ?>
<!--                        </h4>-->

                    </div>
                </div>
                <div class="disfrutar isla-background">
                    <div class="concepto-texto-container">
                        <div class="concepto-texto">
                            <span><?php echo $isla_disfrutar_texto ?></span>
                        </div>
                    </div>
                    <div class="concepto-titulo-container <?php echo $idioma_ext?>isla-disfrutar-titulo">
<!--                        <h4 class="concepto-titulo">-->
<!--                                --><?php //echo $isla_disfrutar_titulo ?>
<!--                        </h4>-->

                    </div>

                </div>
            </div>
        </div>
    </div>
</div>

<div class="container-fluid conceptos hide-until-dom-ready visible-sm visible-xs hidden-lg hidden-md">
                <div class="sentir row">

                    <div class="concepto-titulo-container col-sm-12">
                        <h4 class="concepto-titulo">
                          <?php echo $isla_sentir_titulo ?>
                        </h4>

                    </div>
                    <div class="concepto-texto-container col-sm-12">
                        <div class="concepto-texto">
                            <span><?php echo $isla_sentir_texto ?></span>
                        </div>
                    </div>
                </div>
                <div class="descubrir row">

                    <div class="concepto-titulo-container col-sm-12">
                        <h4 class="concepto-titulo">
                          <?php echo $isla_descubrir_titulo ?>
                        </h4>

                    </div>
                    <div class="concepto-texto-container col-md-12">
                        <div class="concepto-texto">
                            <span><?php echo $isla_descubrir_texto ?></span>
                        </div>
                    </div>
                </div>
                <div class="crear row">

                    <div class="concepto-titulo-container col-sm-12">
                        <h4 class="concepto-titulo">
                          <?php echo $isla_crear_titulo ?>
                        </h4>

                    </div>
                    <div class="concepto-texto-container col-sm-12">
                        <div class="concepto-texto">
                            <span><?php echo $isla_crear_texto ?></span>
                        </div>
                    </div>
                </div>
                <div class="disfrutar row">

                    <div class="concepto-titulo-container col-sm-12">
                        <h4 class="concepto-titulo">
                          <?php echo $isla_disfrutar_titulo ?>
                        </h4>

                    </div>

                    <div class="concepto-texto-container col-sm-12">
                        <div class="concepto-texto">
                            <span><?php echo $isla_disfrutar_texto ?></span>
                        </div>
                    </div>
                </div>
    </div>
</div>