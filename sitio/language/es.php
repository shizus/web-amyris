<?php
// menu 1
$amyris="AMYRIS";
$valores="VALORES";

//menu2

$fragancias="FRAGANCIAS";
// menu3
$expertise="EXPERTISE";
$nuestro_estandarte="Nuestro estandarte es la calidad de nuestros productos y de nuestros procedimientos. Somos estrictos en
cada detalle del proceso de fabricación y entrega. Por ello trabajamos con tecnología de última
generación y con técnicas verificadas mediante prácticas eficaces.";

$idi="I+D+I";
$nuestro_departamento="Nuestros departamento de
innovación trabaja incansablemente
investigando para ofrecer nuevas
sensaciones y propuestas, donde se
evalúan constantemente los procesos
y se aplican a nuevas tecnologías,
herramientas y tendencias que
logren generar un producto
diferencial.";
//menu4

$sustentabilidad="SUSTENTABILIDAD";

//menu5

// subtitulos menu desplegable
//----------------------------


$subtitulomenu1_quienes="&iquest;QUI&Eacute;NES SOMOS?";
$subtitulomenu1_valores="VALORES";
$subtitulomenu1_porque="&iquest;POR QU&Eacute; AMYRIS?";

//menu2

$subtitulomenu2_fragancias="FRAGANCIAS";
$subtitulomenu2_sistemaconceptual="SISTEMA CONCEPTUAL";
$subtitulomenu2_productos="PRODUCTOS";


// menu3


$subtitulomenu3_expertise="EXPERTISE";
$subtitulomenu3_desarrollo="DESARROLLO Y CREACI&Oacute;N";
$subtitulomenu3_infraestructura="INFRAESTRUCTURA";
$subtitulomenu3_control="CONTROL DE CALIDAD";
$subtitulomenu3_comercio="COMERCIO EXTERIOR";


//menu4

$subtitulomenu4_sustentabilidad="SUSTENTABILIDAD";
$subtitulomenu4_enfoque="ENFOQUE SUSTENTABLE";
$subtitulomenu4_operamos="&iquest;COMO OPERAMOS?";
$subtitulomenu4_valoresm="VALORES";



//menu5

$subtitulomenu5_innovaciones="INNOVACIONES";
$subtitulomenu5_departamento="DEPARTAMENTO DE IDEAS, DESARROLLO E INNOVACI&Oacute;N";
$subtitulomenu5_aprendeosinnovar="APRENDEMOS PARA INNOVAR";


$botoncontacto="CONTACTO";




//index

$bienvenidos="BIENVENIDOS A";
$creamos_fragancias="CREAMOS FRAGANCIAS";
$creamos_sensaciones="CREAMOS SENSACIONES";

$fabricamos="Creamos fragancias que transmiten emociones y conectan con la sensibilidad del consumidor";
$logramos="Brindamos excelencia para concretar la imaginación olfativa";
$isla_crear_titulo="crear";
$isla_crear_texto="NUESTRAS FRAGANCIAS SON A MEDIDA DEL CLIENTE, DISE&Ntilde;ADAS Y DESARROLLADAS EN FORMA EXCLUSIVA BRINDANDO EXCELENCIA Y CALIDAD. PERMITIMOS QUE NUESTROS CLIENTES PARTICIPEN EN EL PROCESO  DE CREACI&Oacute;N DEL DISE&Ntilde;O Y ASESORAMOS SOBRE LA MEJOR OPCI&Oacute;N." ;

$isla_sentir_titulo="sentir";
$isla_sentir_texto="LOS AROMAS NOS RECUERDAN UN MOMENTO ESPECIAL DE NUESTRA VIDA Y AL SENTIRLOS, PUEDE GENERARNOS RISA, REGOCIJO, ALEGR&Iacute;A, NOSTALGIA O TRISTEZA. NUESTRO PRINCIPAL OBJETIVO ES INTERPRETAR ESAS SENSACIONES EN UNA FRAGANCIA." ;

$isla_disfrutar_titulo="disfrutar";
$isla_disfrutar_texto="ENCONTRAR LA FRAGANCIA ADECUADA TE GENERA REGOCIJO, ALEGR&Iacute;A Y EMOCI&Oacute;N. EN AMYRIS TE INVITAMOS A DISFRUTAR ESE MOMENTO CON PRODUCTOS DE LA M&Aacute;S ALTA CALIDAD." ;

$isla_descubrir_titulo="descubrir";
$isla_descubrir_texto="SOMOS UN EQUIPO DE TRABAJO PREPARADO PARA INTERPRETAR LA IMAGINACI&Oacute;N OLFATIVA DEL CLIENTE, CONECTAR CON LA SENSIBILIDAD Y COMPRENDER LOS SENTIDOS Y EMOCIONES.";

$amyris_es_armonia="AMYRIS ES ARMON&Iacute;A EN EL AIRE";
$ofrecemos_excelencia="OFRECEMOS EXCELENCIA EN LA CREACI&Oacute;N Y ELABORACI&Oacute;N DE FRAGANCIAS" ;
$contacto="CONTACTO";

$fragancias_finas_titulo="Fragancias Finas";
$fragancias_finas_detalle="Buscamos ofrecerle al cliente la más alta calidad de la fragancia
                                                    creada.";


$cuidado_personal_titulo="Cuidado Personal y <br>Productos de Tocador";
$cuidado_personal_detalle="Asumimos el compromiso de ofrecerle al consumidor productos que al momento
                                                    de percibir los aromas, pueda disfrutar.";


$amyris_proceso_desarrollo="AMYRIS CUBRE TODO EL PROCESO DE DESARROLLO CON LA ÚLTIMA TECNOLOG&Iacute;A, PROFESIONALES ALTAMENTE CAPACITADOS Y UN " ;
$segundo_cuadrado2="DEPARTAMENTO EXCLUSIVO DE IDEAS, DESARROLLO E INVESTIGACI&Oacute;N";
$sustentabilidad2="NUESTRA PREMISA ES GARANTIZAR EL CUIDADO Y LA REDUCCI&Oacute;N DEL IMPACTO AMBIENTAL EN TODOS LOS PROCESOS INVOLUCRADOS EN LA CREACI&Oacute;N Y EL DESARROLLO DE NUESTRAS FRAGANCIAS.  ";

$cuidadohogar="Cuidado del Hogar <br>y Ambientales" ;
$cuidadohogardetalle="Ofrecemos productos que están relacionados con la sensación de seguridad,
                                                    confort, pertenencia y calma.";

$limpieza="Limpieza" ;
$limpiezadetalle="Buscamos que el bienestar generado remita a la tranquilidad de percibir un
                                                    hogar limpio." ;

$cosmetica="Cosm&eacute;tica Automotor";
$cosmeticadetalle="Ofrecemos fragancias que recuerden ese momento de alegría que se obtiene
                                                con un auto nuevo.";

// quienes somos

$el_valor_conectar_sentidos="EL VALOR DE CONECTAR SENTIDOS";
$inspirando_felicidad="Inspirando felicidad y bienestar";
$quienes_somos_valores_por_que="Quienes Somos / Valores / &iquest;Por qu&eacute; AMYRIS?";
$generamos_tendencias="GENERAMOS TENDENCIAS EN <BR> FRAGANCIAS Y CUIDADO PERSONAL";

$tomamos_proyecto_desafio="Tomamos cada proyecto como un desafío único y para que ello sea posible, investigamos, actualizamos los ingredientes, incorporamos materias primas y nos adaptamos a las tendencias mundiales. " ;
$equipo_trabajo_amyris="El equipo de trabajo de AMYRIS S.A. asesora al cliente en todo el desarrollo y tiene como principal objetivo buscar excelencia y calidad en su producto, por ello trabajamos en conjunto con varias marcas líderes y somos considerados como un proveedor destacado";
$fragancia_creada_medida="UNA FRAGANCIA CREADA A MEDIDA OTORGA IDENTIDAD Y CADA DESARROLLO ES UN DESAFÍO ÚNICO." ;
$empresa_familiar="Amyris es una EMPRESA FAMILIAR, con una larga trayectoria y vasta experiencia, dedicada a la
investigación, desarrollo, producción, elaboración, innovación y creación de fragancias para la industria. " ;
$distribuidores_latinoamerica="Poseemos presencia en todo el territorio nacional como también en el internacional. Disponemos de distribuidores en el mercado latinoamericano exclusivamente seleccionado y capacitado para mantener la calidad de nuestros productos." ;
$nuestro_expertise="Nuestro expertise se apoya en la búsqueda constante de soluciones sensoriales innovadoras, buscamos cautivar los sentidos para representarlos en una fragancia, explorando y descubriendo cuáles emociones nos conmueven, posibilitando
así el objetivo de experimentar una vivencia única. " ;

$honestidad="HONESTIDAD";
$respesto_sinceridad ="El respeto y la sinceridad son pilares fundamentales de nuestra empresa.
Garantizamos a nuestros clientes transparencia en nuestros procesos desde el
inicio hasta el final." ;
$etica="ÉTICA";
$proyecto_compromiso_cliente="Cada proyecto implica un compromiso con el cliente y esa obligación que
tomamos hace que cada desarrollo sea único e irrepetible. Nos caracteriza una
conducta de fidelidad, confianza y lealtad sobre el trabajo realizado." ;
$sinergia="SINERGIA";
$trabajamos_conjunto="Trabajamos en conjunto con los clientes como socios estratégicos." ;

$por_que_amyris_pregunta="¿ Por Qué Amyris ?";
$nuestra_vision="NUESTRA VISIÓN SE BASA EN 4 PILARES FUNDAMENTALES QUE SON LA
ESENCIA DE QUIÉNES SOMOS Y TODO AQUELLO QUE NOS REPRESENTA." ;
$equipo="EQUIPO" ;

$personas_trabajan_amyris="Cada uno de las personas que trabajan en AMYRIS S.A. es una pieza fundamental del proceso. El trabajo en equipo permite capacitarse, adquirir experiencia, fluidez de creatividad para garantizarle al cliente
excelencia y calidad. " ; 
$compromiso="COMPROMISO";
$buscamos_alternativas="Buscamos alternativas, brindamos ideas y trabajamos en conjunto con el cliente. Prestamos asesoramiento sobre la
conveniencia de utilizar una fragancia de nuestra creación o embarcarnos a realizar un desarrollo a medida. " ;
// Hasta acá ya borré todas las que no se usan
$calidad="CALIDAD" ;
$garantizamos_calidad="Garantizamos a nuestros clientes la calidad de cada uno de los ingredientes que componen la fragancia.
Para ello disponemos: " ; 
$innovacion="INNOVACI&Oacute;N" ;
$inquietud_explorar="La inquietud por explorar nuevas alternativas y el deseo de superaci&oacute;n constante nos lleva a querer estar siempre  a la vanguardia en ofrecer resultados creativos e innovadores.Nos ocupamos de actualizarnos y anticiparnos a las
tendencias mundiales para ofrecer al cliente un abanico de variedades.
"; 
$innovaciones="INNOVACIONES";
$proceso_desarrollo="AMYRIS CUBRE TODO EL PROCESO DE DESARROLLO DE FRAGANCIAS CON LA ÚLTIMA TECNOLOGÍA, PROFESIONALES ALTAMENTE CAPACITADOS Y UN DEPARTAMENTO EXCLUSIVO DE IDEAS, DESARROLLO E INVESTIGACIÓN. " ;

$amyris_inspiracion="AMYRIS ES INSPIRACI&Oacute;N";
$laboratorio_muestras="Laboratorio de muestras donde se realizan las pruebas y aplicaciones sobre
el producto del cliente garantizando que la fragancia creada cumpla con los parámetros solicitados." ;
$laboratorio_control_calidad="Laboratorio de control de calidad que analiza rigurosamente el producto desde el ingreso a
nuestra empresa hasta su salida. Todo el proceso de producción está controlado por los máximos standars de calidad" ;

//fragancias
$fragancias_titulo="SISTEMA CONCEPTUAL AMYRIS";
$valor_transmitir_emociones="EL VALOR DE TRANSMITIR EMOCIONES";
$vinculando_nuestras_fragancias="vinculando nuestras fragancias con la sensibilidad del consumidor";
$sistema_conceptual_amyris="Sistema Conceptual Amyris / Productos";
$amyris_creacion_innovacion="AMYRIS ES CREACIÓN E INNOVACIÓN" ;
$transformamos_fragancia="TRANSFORMAMOS LA FRAGANCIA  EN UN SELLO PERSONAL " ;
$desde_nuestros_inicios="Desde nuestros inicios, nuestro objetivo siempre fue la búsqueda de soluciones sensoriales que nos permita
desarrollar y crear fragancias únicas. Esto nos llevó a desarrollar un sistema conceptual de trabajo:";
$sentir_descripcion="LOS AROMAS NOS RECUERDAN UN MOMENTO ESPECIAL DE NUESTRA VIDA Y AL SENTIRLOS PUEDE GENERARNOS RISA, REGOCIJO, ALEGRÍA, NOSTALGIA O TRISTEZA. NUESTRO PRINCIPAL OBJETIVO ES INTERPRETAR ESAS SENSACIONES EN UNA FRAGANCIA.";


$descubrir_descripcion="SOMOS UN EQUIPO DE TRABAJO PREPARADO PARA INTERPRETAR LA IMAGINACIÓN OLFATIVA DEL CLIENTE, CONECTAR CON LA SENSIBILIDAD Y COMPRENDER LOS SENTIDOS Y EMOCIONES.";

$oler_descripcion="NUESTRAS FRAGANCIAS SON A MEDIDA DEL CLIENTE, DISEÑADAS Y DESARROLLADAS EN FORMA EXCLUSIVA BRINDANDO EXCELENCIA Y CALIDAD. PERMITIMOS QUE NUESTROS CLIENTES PARTICIPEN EN EL PROCESO DE CREACIÓN DEL DISEÑO Y ASESORAMOS SOBRE LA MEJOR OPCIÓN.";


 $disfrutar_descripcion="LOS AROMAS NOS RECUERDAN UN MOMENTO ESPECIAL DE NUESTRA VIDA Y AL SENTIRLOS PUEDE GENERARNOS RISA, REGOCIJO, ALEGRÍA, NOSTALGIA O TRISTEZA. NUESTRO PRINCIPAL OBJETIVO ES INTERPRETAR ESAS SENSACIONES EN UNA FRAGANCIA.";


$contamos_personal="Contamos con personal de desarrollo y marketing en constante capacitación e investigación para estar siempre estar a la vanguardia de las tendenciales mundiales del mercado de cosmética, fragancias finas, higiene personal, productos para el hogar y productos de limpieza" ;

$consideramos_personal="En Amyris consideramos a cada desarrollo, a cada fragancia especialmente creada como un desafío único. Realizamos controles de calidad, trazabilidad y estabilidad. Trabajamos con un preciso cronograma de tiempos.";

$caja_productos_titulo="FRAGANCIAS FINAS";
$caja_cuidado_personal_titulo="CUIDADO PERSONAL Y PRODUCTOS DE TOCADOR";
$caja_cuidado_hogar_titulo="CUIDADO DEL HOGAR Y AMBIENTALES";
$caja_limpieza_titulo="LIMPIEZA";
$caja_cosmetica_titulo="COSMÉTICA AUTOMOTOR";


$caja_productos_texto_debajo1="En AMYRIS buscamos ofrecerle al cliente la más alta calidad de la fragancia creada, incentivándolo a innovar, brindándole actualización sobre las tendencias a nivel mundial.";
$caja_productos_texto_debajo2="Podemos ofrecer Eau de Perfum, Eau de Toillete, Colonias, fragancias femeninas, masculinas, para adolecentes, niños y bebés.";

$caja_cuidado_personal_texto_debajo1="Asumimos el compromiso de ofrecerle al consumidor productos que al momento de percibir los aromas pueda disfrutar.";
$caja_cuidado_personal_texto_debajo2="En esta categoría ofrecemos fragancias para jabones líquidos, productos de tocador, exfoliantes, shampoo, faciales, espumas, sales de baño, cremas, talcos y tinturas.";

$caja_cuidado_hogar_texto_debajo1="El hogar está estrechamente relacionado con una sensación de seguridad, confort, pertenencia y calma.";
$caja_cuidado_hogar_texto_debajo2="Ofrecemos productos para desodorantes ambientales, desodorantes textiles, velas aromáticas, ambientadores y difusores.";

$caja_limpieza_texto_debajo1="Buscamos que el bienestar generado por este tipo de fragancias remita a la pureza, a esa tranquilidad que se genera al percibir estar en un hogar limpio.";
$caja_limpieza_texto_debajo2="Esta clase de fragancias aplican a productos tales como limpiadores para piso, limpiadores en crema, lavandinas, detergentes, suavizantes, productos quita mancha y apresto.";

$caja_cosmetica_texto_debajo1="Momentos como la compra de tu primer auto o cambiarlo por otro más grande generan alegría
			y felicidad. ";
$caja_cosmetica_texto_debajo2="Ofrecemos fragancias que le recuerden ese momento. Lo asesoramos sobre la fragancia adecuada dejando un aroma de alta persistencia.";

$titulo_footer_expertise="CONTAMOS CON TECNOLOGÍA DE ÚLTIMA GENERACIÓN Y TRABAJAMOS CON TÉCNICAS VERIFICADAS Y RIGUROSAS NORMAS INTERNAS
     QUE NOS PERMITEN GENERAR PROCEDIMIENTOS EFICACES.";


$sentir="SENTIR";
$descubrir="DESCUBRIR";
$crear="CREAR";
$disfrutar="DISFRUTAR";

// expertise
$ultima_generacion ="Tecnología de última generación en los laboratorios y sectores productivos";
$valor_brindar_calidad="EL VALOR DE BRINDAR CALIDAD";
$controlando_nuestros_productos="controlando nuestros productos y procedimientos";
$desarrollo_creacion_infraestructura="Desarrollo y Creaci&oacute;n / Infraestructura / Control de Calidad / Comercio Exterior";
$amyris_armonia_aire="AMYRIS ES ARMONÍA EN EL AIRE";
$buscamos_generar_conexion="BUSCAMOS GENERAR UNA CONEXIÓN ENTRE LA FRAGANCIA Y EL CONSUMIDOR";
$desarrollo_creacion="DESARROLLO Y CREACI&Oacute;N";
$nuestros_origines="Desde nuestros orígenes nuestro objetivo fue y sigue siendo
crecer junto al cliente ofreciendo creación, desarrollo,
innovación, actualización del producto, calidad y originalidad. ";
$textoe8="Contamos con un excelente equipo de profesionales que
contempla cada necesidad y analiza cada detalle en todas las
etapas del proyecto: desde el diseño, la fabricación, los
controles, el envasado hasta la misma entrega. Trabajamos con
un estricto cronograma que nos posibilita cumplir con plazos de
entrega diferenciales y una logística de entrega eficaz.";
$ofrecemos_creacion_desarrollo="OFRECEMOS CREACIÓN, DESARROLLO,
INNOVACIÓN, ACTUALIZACIÓN DEL PRODUCTO,
CALIDAD Y ORIGINALIDAD.";
$buscamos_identificacion="EN AMYRIS, BUSCAMOS SIEMPRE QUE EL CONSUMIDOR OBTENGA UNA IDENTIFICACIÓN ÚNICA CON EL PRODUCTO. ";
$momento_pensar_creacion="Al momento de pensar cada creación, buscamos que el
consumidor obtenga una identificación única con el producto.
Para ello contamos con una amplia cartera de productos que
se actualizan continuamente para adecuarse a las novedades
internacionales. Asimismo, ofrecemos en nuestros productos
componentes e insumos especiales, materias primas naturales
y especialidades.  ";
$conservar_cliente_crecer="Nos interesa conservar al cliente y crecer junto a él, es política
de nuestra empresa realizar un seguimiento del producto para
estar al tanto de su aceptación en el mercado y la
actualización del mismo.";
$amyris_se_encuentra="Amyris se encuentra ubicada en Munro, Provincia de Bs. As. Nuestras modernas instalaciones –oficinas administrativas, comerciales,
laboratorio, depósito, departamento de control de calidad– cumplen con todas las normas de seguridad e higiene establecidas, con
las legislaciones municipales y con las normas internacionales vigentes.";
$asimismo_proceso="Asimismo, todo el proceso de fabricación sigue un riguroso control de calidad con certificados de análisis y hojas de seguridad del
producto. La planta se encuentra controlada tanto interna como externamente, con un equipo que revisa regularmente todos los
procesos, no sólo para evitar riesgos, si no para la seguridad del personal y conservación de los productos. ";
$control_calidad="CONTROL DE CALIDAD";
$debemos_destacar="Debemos destacar, como un pilar
fundamental de nuestra empresa, LA
CALIDAD de nuestros productos y
nuestros procedimientos.  ";
$estandarte_de_calidad="Estandarte de Calidad Amyris";


$articulo1="En la "; 
$articulo2="En los ";
$articulo3="En el "; 
$articulo4="En ";

$estandarte_de_calidad_creacion="Creación";
$estandarte_de_calidad_procesos="Procesos de Fabricación";
$estandarte_de_calidad_cronograma="Cronograma y los Plazos";
$estandarte_de_calidad_control="Control de Calidad";
$estandarte_de_calidad_certificado="Certificado de Análisis";
$estandarte_de_calidad_servicio="Servicio de Postventa";








$importamos_cada_componente="Importamos cada uno de los componentes e insumos para
elaborar las fragancias. Cada insumo que luego se va a
aplicar en la fragancia, está previamente controlado
(analizado olfativamente, índice de densidad, cromatografía,
color, olfativo e índices de refracción). Cada ingrediente de la
fragancia tiene una trazabilidad, ya que todo tiene un lote
identificado para cumplir siempre los parámetros de calidad.";

$emitimos_certificado="Al finalizar, emitimos un CERTIFICADO DE ANÁLISIS y HOJA DE
SEGURIDAD que se entrega junto con la mercadería y que
servirá como garantía al cliente.";

$respeto_por_cliente="El respeto por el cliente es la clave de nuestro &eacute;xito.";
$somos_estrictos="SOMOS ESTRICTOS EN CADA DETALLE DEL PROCESO DE
DESARROLLO, FABRICACIÓN Y ENTREGA. ESTA ES UNA DE
LAS PRINCIPALES RAZONES POR LA CUAL LAS EMPRESAS
LÍDERES CONFÍAN EN NOSOTROS. ";
$comercio_exterior="COMERCIO EXTERIOR";
$base_buenos_aires="Teniendo base en Buenos Aires, Amyris posee presencia en todo el territorio argentino y est&aacute; presente tambi&eacute;n en el mercado internacional, principalmente con una fuerte incidencia en varios pa&iacute;ses de Am&eacute;rica Latina, siendo adem&aacute;s partners de empresas l&iacute;deres del rubro.";
$proces_expansion="Asimismo, nos encontramos en un proceso de expansi&oacute;n inmediata que llega al continente asi&aacute;tico mediante una red profesional de distribuidores.";
$fabricamos_fragancia_apropiada="FABRICAMOS LA FRAGANCIA APROPIADA
PARA CADA PRODUCTO, BUSCANDO LA IDENTIFICACIÓN
CON EL CONSUMIDOR A TRAVÉS DE UN SELLO PERSONAL.";


//susten
$valor_confianza="EL VALOR DE LA CONFIANZA";
$garantizando_cuidado = "garantizando el cuidado de nuestros procesos";
$valores_barra="Valores / ";
$enfoque_sustentable_breadcrumbs=" Enfoque Sustentable /";
$como_operamos=" C&oacute;mo Operamos";

$amyris_etica="AMYRIS ES &Eacute;TICA";
$trabajamos_mas_altos="TRABAJAMOS CON LOS M&Aacute;S ALTOS EST&Aacute;NDARES EN SEGURIDAD E HIGIENE";

$trabajamos_responsable="En Amyris, trabajamos de manera responsable y siguiendo
los procedimientos escritos. Buscamos que el ambiente de
trabajo sea un lugar seguro, higiénico y agradable. Esto es
una preocupación constante que sabemos influye en toda
la cadena de producción, hasta llegar al consumidor final. ";

$cumplir_todas_normas="Por eso, además de cumplir con todas las normas vigentes,
controlamos cuidadosamente los procedimientos y circuitos
de trabajo beneficiando así a nuestros clientes, que
reciben productos con la más alta calidad. ";

$enfoque_sustentable="ENFOQUE SUSTENTABLE";
$nuestro_compromiso="Nuestro compromiso está en garantizar el cuidado y la
reducción del impacto ambiental en todos los procesos. ";

$controlamos_responsablemente="CONTROLAMOS RESPONSABLEMENTE<br>TODOS LOS PROCESOS.";
$conocemos_detalle="Conocemos en detalle los ingredientes y la procedencia de
nuestras materias primas: nos abastecemos y trabajamos con
ellos de manera responsable, previniendo de esta manera
toda posible contaminación. ";

$somos_conscientes="Somos conscientes de los impactos y su importancia en la
sociedad, por eso nos ayuda conocer y dialogar con las
empresas involucradas en la cadena trabajo. Tener este
conocimiento nos permite generar una eficaz estrategia
sustentable y ser colaborativos y cooperativos en todo
momento. Nuestra política de gestión es buscar siempre hacer
negocios de manera sostenible. ";

$cuidado_salud_bienestar="EL CUIDADO, LA SALUD Y EL BIENESTAR SON EL EJE DE NUESTRA VISI&Oacute;N SUSTENTABLE.";
$capacitamos_personal="CAPACITAMOS A NUESTRO PERSONAL PARA MINIMIZAR O SUPRIMIR
POSIBLES IMPACTOS Y PARA UN CORRECTO MANEJO DE DESECHOS
DENTRO DE LA PLANTA DE PRODUCCIÓN.";
$seguridad_higiene="SEGURIDAD E HIGIENE";
$cumplimos="Cumplimos de manera estricta con los
controles municipales y capacitamos
constantemente al personal en seguridad
para minimizar todo tipo de riesgo.";
$impacto_ambiental=" IMPACTO AMBIENTAL";
$hacemos_estudios="Hacemos estudios y controles de impacto
ambiental con el objetivo de mejorar
continuamente nuestros estándares,
reforzando nuestra performance operacional
para ser más eficientes.";
$seguridad_higiene="SEGURIDAD E HIGIENE";
$desechos="DESECHOS";
$controlamos_residuos="Controlamos rigurosamente nuestros residuos,
los cuales requieren distintos tipos de cuidados
para ser extraídos de la planta. Por eso,
tenemos todos los certificados correspondientes
obtenidos luego de que cada desecho es
generado. Así evitamos alteraciones en los
productos, preservamos nuestras instalaciones y
aseguramos el bienestar de nuestra gente.";

$quienes_somos="QUIENES SOMOS";
$exclusividad_asesoramiento="EN AMYRIS, TRABAJAMOS A LA MEDIDA DEL CLIENTE CON EXCLUSIVIDAD ABSOLUTA Y UN ASESORAMIENTO TOTAL EN LA FABRICACI&Oacute;N DEL PRODUCTO, PARA EL MEJOR RESULTADO FINAL.";


// innovaciones

$valor_ideas="EL VALOR DE LAS IDEAS";
$otorgando_innovacion="otorgando innovación y originalidad";
$aprendemos_innovar_departamento="Aprendemos para Innovar / Departamento de Ideas, Desarrollo e Innovaci&oacute;n";
$amyris_futuro="AMYRIS ES FUTURO";
$brindamos_soluciones_originales="BRINDAMOS SOLUCIONES ORIGINALES, ANTICIP&Aacute;NDONOS A LAS &Uacute;LTIMAS TENDENCIAS";
$aprendemos_innovar="APRENDEMOS PARA INNOVAR";
$ofrecer_soluciones_creativas="Para ofrecer soluciones creativas, viajamos, nos informarnos y concurrimos a
congresos internacionales. Capacitamos constantemente a nuestro personal,
le damos la posibilidad de crecer y brindar alternativas para mejorar la forma
de trabajo. Esto nos permite adelantarnos y estar siempre a la vanguardia en
ideas y propuestas otorgando originalidad a los desarrollos de los clientes";

$buscamos_innovar_y="BUSCAMOS INNOVAR Y ";
$dar_originalidad="DAR ORIGINALIDAD A LAS FRAGANCIAS.";
$departamento_ideas="DEPARTAMENTO DE IDEAS, DESARROLLO E INNOVACI&Oacute;N";
$consumidor_fuente_inspiracion="En Amyris, el consumidor es la fuente de inspiración. Nuestro objetivo es que la fragancia deseada se pueda obtener con la más alta
calidad y originalidad. Contamos con un Departamento de IDEAS, DESARROLLO E INNOVACIÓN integrado por profesionales de
distintas especializaciones que trabajan investigando para ofrecer nuevas propuestas, donde se evalúan constantemente los procesos y
se aplican a nuevas tecnologías, herramientas y tendencias que logren generar un producto diferencial, único para el cliente.";
$idi_nos_permite="EL DEPARTAMENTO I+D+I NOS PERMITE OFRECER UN SERVICIO COMERCIAL COMPLETO Y EFICIENTE";
$generar_nuevos_productos="AL GENERAR NUEVOS PRODUCTOS Y/O AL REFORMULAR U OPTIMIZAR LOS YA EXISTENTES, SIEMPRE APOYADO POR NUESTRAS &Aacute;REAS T&Eacute;CNICO-COMERCIALES";
$congresos_internacionales="CONGRESOS INTERNACIONALES";

$inno_como_trabaja="¿CÓMO TRABAJA NUESTRO DEPARTAMENTO I+D+I?";


$asistimos_anualmente="Asistimos anualmente a los más
importantes eventos mundiales
donde se reúnen perfumistas,
vendedores, fabricantes y
proveedores de ingredientes, para
observar las tendencias en el
mundo de las fragancias y ofrecer
así, ideas innovadoras y creativas
a nuestros clientes.";

$investigaciones_capacitaciones="INVESTIGACIONES Y CAPACITACIONES";
$invertimos_continuamente="Invertimos continuamente en la capacitación de
nuestro personal para estudiar el impacto de
nuestros productos en el consumidor final y
generar productos duraderos y destacados.
Investigamos los productos del mercado y el
universo de las fragancias para dar
originalidad a la fragancia. Esto nos permite
anticiparnos a lo que el cliente quiere, creando
nuevas oportunidades durante el proceso.";
$ideas_e_innovaciones="IDEAS E INNOVACIONES";
$pensamos_trabjamos_equipo="Pensamos y trabajamos en equipo.
Tanto nuestros perfumistas como
nuestros vendedores y
desarrolladores, aportan ideas al
I+D+I para generar productos
nuevos y creativos.";
$necesita_consulta_comentario="&iquest;NECESITA HACER ALGUNA CONSULTA O COMENTARIO? ESCR&Iacute;BANOS PARA QUE PODAMOS AYUDARLO.";

//contacto
$valor_comunicados="EL VALOR DE ESTAR COMUNICADOS";
$generamos_vinculos="Generamos v&iacute;nculos duraderos y confiables";
$donde_estamos="&iquest;D&oacute;nde estamos? / H&aacute;ganos su consulta";
$conocer_necesidades="En Amyris, nos interesa conocer las necesidades y la opini&oacute;n de cada cliente.<br>
Queremos que se involucre en cada detalle del proceso de creaci&oacute;n y fabricaci&oacute;n,<br>
para que podamos as&iacute;, atender a cualquier sugerencia.";
$planta_produccion="Planta de Producci&oacute;n y Oficina de Ventas";
$haganos_consulta="H&Aacute;GANOS SU CONSULTA";
$nombre_apellido="NOMBRE Y APELLIDO";
$email_label="EMAIL";
$telefono_label="TEL&Eacute;FONO";
$empresa_label="EMPRESA";
$mensaje_label="MENSAJE";
$enviar="ENVIAR";
$mensaje_enviado="El mensaje ha sido enviado.¡Muchas Gracias! En breve nos pondremos en contacto.";


$donde_estamos_consulta="DONDE ESTAMOS";


$amyris_estrictos_detalle="EN AMYRIS, SOMOS ESTRICTOS EN CADA DETALLE DEL PROCESO DE FABRICACI&oacute;N Y ENTREGA. ESTA ES UNA DE LAS
PRINCIPALES RAZONES POR LA CUAL EMPRESAS L&iacute;DERES CONFÍAN EN NOSOTROS.";
$gracias_por_consulta="Gracias por su consulta";
$nos_comunicaremos="En breve nos estaremos comunicando con ud.";

$cambiarIdioma="Cambiar Idioma";
$inicio="Inicio";
$spanish="Español";
$english="Inglés";
$french="Francés";


// send-mail

$output_nombre_corto="Nombre vacio o demasiado corto!";
$output_email_invalido="Igrese un email valido!";
$output_solonumeros="Compañia vacia o demasiado corta";
$output_compania_invalida="Ingrese solo numeros";
$output_sujeto_requerido="Sujeto requerido";
$output_mensaje_corto="Mensaje demasiado corto!";
$output_mensaje_error="No pudo mandar el mensaje. Compruebe su configuracion.";
$output_mensaje_exito="Gracias por su mensaje";


// footer

$derechos_reservados="Todos los derechos reservados ";

// formulario desplegable

$formulario_conocernos="¿Querés conocernos?";
$formulario_tusdatos="Dejanos tus datos";
$formulario_datosobligatorios="* Datos Obligatorios";
$formulario_mensajeenviado="El mensaje ha sido enviado.";
$formulario_mensajegracias="¡Muchas gracias!";
$formulario_mensajecontacto="En breve nos pondremos en contacto.";
$formulario_botonenviar="Enviar";

$formulario_camponombre="Nombre*";
$formulario_campoemail="Email*";
$formulario_campotelefono="Teléfono*";
$formulario_campoempresa="Empresa*";
$formulario_campomensaje="Mensaje*";

$formulario_telefonoescucharte="Queremos escucharte";
$formulario_telefonoconsulta="¿Tenés una consulta?";
$formulario_telefonosugerencia="¿Tenés una sugerencia?";
$formulario_telefonocontarnos="¿Querés contarnos acerca de tu idea?";
$formulario_telefononecesidades="Nos interesa conocer las necesidades y la opinión de cada cliente, por eso siempre estamos acompañando cada proyecto, ayudándolos a crecer día a día.";
$formulario_telefonollamanos="¡Llamanos!";


//formulario mapa

$formulariomapa_donde="¿Dónde estamos?";
$formulariomapa_ubicacion="Nuestra Planta de Producción y Oficina de Ventas se encuentra en Cajaraville 2902 y/o Curapaligüe 6510, Munro (1605, Provincia de Buenos Aires)";
$footer_ubicacion="Cajaraville 2902 y/o Curapaligüe 6510, Munro (1605, Provincia de Buenos Aires)";
$ubicacion = "Cajaraville 2902 y/o Curapaligue 6510,<br> Munro (1605) Pcia. de Bs. As";


// No tocar
$idioma_ext="";